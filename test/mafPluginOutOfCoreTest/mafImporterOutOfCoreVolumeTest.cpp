...
void testVMEAddObserver::vmeImported(mafCore::mafObjectBase *vme) {
    ...
    mafProxy<mafVolume> *output = mafProxyPointerTypeCast(mafVolume, dataset->dataValue());
    //QVERIFY(data); //need to check something!!

}

/**
 Class name: mafImporterOutOfCoreTest
 This class implements the test suite for mafImporterVTK.
 */
class mafImporterOutOfCoreVolumeTest: public QObject {
    Q_OBJECT
...
private Q_SLOTS:
    ...
    void importOutOfCoreFile();

private:
    QString m_OutOfCoreFile; ///< VTK filename to import.
...
void mafImporterOutOfCoreVolumeTest::initializeTestData() {
...
MAF_REGISTER_TEST(mafImporterOutOfCoreVolumeTest);
#include "mafImporterOutOfCoreVolumeTest.moc"
