...
import math
import Executer
from Rules.NamingConventionRules import FunctionNamingRule
from Rules.NamingConventionRules import OneClassPerFileDeclarationRule
from Rules.NamingConventionRules import VariableNamingRule

class NamingConventionRulesQA():
    def __init__(self):
    ...
    def FunctionNamingRule(self, dirPath, file):
        rule = FunctionNamingRule.FunctionNamingRule()
        rule.setRuleParameters("function","\A((?![a-z])\w+)")
        xmlFile = self.CheckClassFile(dirPath, file, rule)
        if(xmlFile != None):
          self.Classes.append(xmlFile)

    def OneClassPerFileDeclarationRule(self, dirPath, file):
        rule = OneClassPerFileDeclarationRule.OneClassPerFileDeclarationRule()
        rule.setRuleParameters(None)
        xmlFile = self.CheckClassFile(dirPath, file, rule)
        if(xmlFile != None):
          self.Classes.append(xmlFile)

    def VariableNamingRule(self, dirPath, file):
        rule = VariableNamingRule.VariableNamingRule()
        rule.setRuleParameters("variable","\A((?!m_[A-Z])\w+)")
        xmlFile = self.CheckClassFile(dirPath, file, rule)
        if(xmlFile != None):
          self.Classes.append(xmlFile)
    ...
    def CheckSources(self,baseDirectory, outputDirectoryCheck, ruleMethod):
        ...
        self.FileOutput.write("Check that variables or functions respect the naming convention" + "\n")
        self.FileOutput.write("</ruleDescription>\n")
...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        ...
        nc = NamingConventionRulesQA()
        nc.SetFilePattern("\Aclass.*\.xml")
        nc.CheckSources(sys.argv[1],  sys.argv[2],"FunctionNamingRule")
        nc.CheckSources(sys.argv[1],  sys.argv[2],"OneClassPerFileDeclarationRule")
        nc.CheckSources(sys.argv[1],  sys.argv[2],"VariableNamingRule")
