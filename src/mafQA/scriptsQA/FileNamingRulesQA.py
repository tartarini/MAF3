...
import math
import Executer
from Rules.FileNamingRules import FileNamingRule

class FileNamingRulesQA():
    def __init__(self):
    ...
    def FileNamingRule(self, dirPath, file):
        rule = FileNamingRule.FileNamingRule()
        rule.setRuleParameters("FileNamingDictionary.txt")
    ...
    def CheckSources(self,baseDirectory, outputDirectoryCheck, ruleMethod):
        ...
        self.FileOutput.write("Check that filename respect the naming convention" + "\n")
        self.FileOutput.write("</ruleDescription>\n")
...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        ...
        nc = FileNamingRulesQA()
        nc.SetFilePattern("\Aclass.*\.xml")
        nc.CheckSources(sys.argv[1],  sys.argv[2],"FileNamingRule")
