...
import math
import Executer
from Rules.StatisticRules import LicenseHeaderRule
from Rules.StatisticRules import LinesNumberRule

class StatisticRulesQA():
    def __init__(self):
    ...
    def LicenseHeaderRule(self, dirPath, file):
        rule = LicenseHeaderRule.LicenseHeaderRule()
        rule.setRuleParameters("See Licence at: http://tiny.cc/QXJ4D")
        xmlFile = self.CheckClassFile(dirPath, file, rule)
        if(xmlFile != None):
          self.Classes.append(xmlFile)

    def LinesNumberRule(self, dirPath, file):
        rule = LinesNumberRule.LinesNumberRule()
    ...
    def CheckSources(self,baseDirectory, outputDirectoryCheck, ruleMethod):
        ...
        self.FileOutput.write("Check Statistics for each class" + "\n")
        self.FileOutput.write("</ruleDescription>\n")
...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        ...
        nc = StatisticRulesQA()
        nc.SetFilePattern("^(?!moc_).*\.(h|cpp|txx|cxx)$")
        nc.CheckSources(sys.argv[1],  sys.argv[2],"LicenseHeaderRule")
        nc.CheckSources(sys.argv[1],  sys.argv[2],"LinesNumberRule")
