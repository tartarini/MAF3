...
import math
import Executer
from Rules.GuideCreatorRules import GuideCreatorRule

class GuideCreatorRulesQA():
    def __init__(self):
    ...
    def GuideCreatorRule(self, dirPath, file):
        rule = GuideCreatorRule.GuideCreatorRule()
    ...
    def CheckSources(self,baseDirectory, outputDirectoryCheck, ruleMethod):
        ...
        self.FileOutput.write("Show comments on tests" + "\n")
        self.FileOutput.write("</ruleDescription>\n")
...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        ...
        nc = GuideCreatorRulesQA()
        nc.SetFilePattern(".*\.(cpp|txx|cxx)$")
        nc.CheckSources(sys.argv[1],  sys.argv[2],"GuideCreatorRule")
