...
import math
import Executer
from Rules.DocumentationRules import ClassDocumentationRule
from Rules.DocumentationRules import ClassMethodsDocumentationRule

class DocumentationRulesQA():
    def __init__(self):
    ...
    def ClassDocumentationRule(self, dirPath, file):
        rule = ClassDocumentationRule.ClassDocumentationRule()
        rule.setRuleParameters(None)
        xmlFile = self.CheckClassFile(dirPath, file, rule)
        if(xmlFile != None):
          self.Classes.append(xmlFile)

    def ClassMethodsDocumentationRule(self, dirPath, file):
        rule = ClassMethodsDocumentationRule.ClassMethodsDocumentationRule()
        rule.setRuleParameters("function")
        xmlFile = self.CheckClassFile(dirPath, file, rule)
        if(xmlFile != None):
          self.Classes.append(xmlFile)
    ...
    def CheckSources(self,baseDirectory, outputDirectoryCheck, ruleMethod):
        ...
        self.FileOutput.write("Check that documentation respect the naming convention" + "\n")
        self.FileOutput.write("</ruleDescription>\n")
...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        ...
        nc = DocumentationRulesQA()
        nc.SetFilePattern("\Aclass.*\.xml")
        nc.CheckSources(sys.argv[1],  sys.argv[2],"ClassDocumentationRule")
        nc.CheckSources(sys.argv[1],  sys.argv[2],"ClassMethodsDocumentationRule")
